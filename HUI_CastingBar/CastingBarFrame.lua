...
function HUI_CastingBarMixin.UpdateIconShown(self)
    ...
    local iconSize = Addon.PP.Scale(Addon:GetValue("UseCastBarIconSize", nil, frameName) and Addon:GetValue("CastBarIconSize", nil, frameName) or self:GetHeight())
    ...
    if self.Icon and self.__iconFrameLeft and iconPos > 1 then
        ...
        Addon.PP.Point(self.__iconFrameLeft, point, self.Background, relPoint, iconPos ~= 3 and Addon.PP.Scale(offsetX) or Addon.PP.Scale(offsetX)*-1, Addon.PP.Scale(offsetY))
        Addon.PP.Size(self.__iconFrameLeft, iconSize)
        self.__iconFrameLeft:Show()
        self.Icon:SetParent(self.__iconFrameLeft)
        self.Icon:SetPoint("CENTER", self.__iconFrameLeft, "CENTER")
        Addon.PP.Size(self.Icon, iconSize)
        self.Icon:Show()
    ...
    if self.__rightIcon and self.__iconFrameRight and iconPos > 1 then
        if iconPos == 4 then
            ...
            Addon.PP.Point(self.__iconFrameRight, "LEFT", self.Background, "RIGHT", Addon.PP.Scale(offsetX) * -1, Addon.PP.Scale(offsetY))
            Addon.PP.Size(self.__iconFrameRight, iconSize)
            ...
            Addon.PP.Size(self.__rightIcon, iconSize)
...
function HUI_CastingBarMixin.SetLook(self, look)
    if not self then return end
    ...
    Addon.PP.Width(self, self.__barWidth)
    Addon.PP.Height(self, self.__barHeight)
    ...
    Addon.PP.Point(self.Text, point, self, relPoint, offsetX, offsetY)

    Addon.PP.Width(self.Text, self.__barWidth * 0.8)
    ...
    do
        ...
        Addon.PP.Point(self.__valueText, timerPoint, self.Background, timerRelPoint, timerOffsetX, timerOffsetY)
    ...
    if self.__showTargetName then
        ...
        Addon.PP.Point(self.__CastTargetNameText, point, self.Background, relPoint, offsetX, offsetY)
...
function HUI_CastingBarMixin.SetStatusBarTexture(self)
    ...
    Addon.PP.DisablePixelSnap(self)
...
function HUI_CastingBarMixin.HandleCastStop(self, event, castID, castComplete, interruptedBy)
    if not self then return end
...
function HUI_CastingBarMixin.OnPlayInterruptAnims(self)
    if not self then return end
...
function HUI_CastingBarMixin.SetCustomColor(self)
...
function HUI_CastingBarMixin.GetTypeInfo(self, barType)
    if not self then return end
...
function HUI_CastingBarMixin.AddStages(self, numStages)
    if not self then return end
    ...
    for i = 1, numStages do
        ...
        Addon.PP.Height(self.StagePips[i].BasePip, self.__barHeight or 10)

    end
end

function HUI_CastingBarMixin.UpdateStage(self)
...
function HUI_CastingBarMixin.OnUpdate(self, elapsed)
    if not self then return end
...
function HUI_CastingBarMixin.ShowSpark(self)
    if not self then return end
    ...
    if not issecretvalue(currentBarType.interrupted) and currentBarType.interrupted then
		...
		Addon.PP.Size(self.Spark, pipWidth, pipHeight)
...
function HUI_CastingBarMixin.PlayFinishAnim(self)
    local frameName = self.boss and "BossTargetFrames" or self:GetName()
...
function HUI_CastingBarMixin.SetColorFill(self)
    HUI_CastingBarMixin.GetTypeInfo(self, "empowered")
end

function HUI_CastingBarMixin.UpdateHighlightImportantCast(self)
    if self.spellID == nil then return end


end

function HUI_CastingBarMixin.AdjustPosition(self)
    if not self then return end
    ...
    self:ClearAllPoints()
    Addon.PP.Point(self, point, parentFrame, relPoint, offsetX, offsetY)
end

function HUI_CastingBarMixin.ProcessShieldBorder(self)
    if not self then return end
    ...
    if self.BorderShield then
        ...
        if shieldAtlas.hide then
            ...
            Addon.PP.Size(self.BorderShield, size)
            ...
            Addon.PP.Point(self.BorderShield, point, self, relPoint, offsetX, offsetY)
        end
    end
end
...
function HUI_CastingBarMixin.OnGetEffectiveType(self, isChannel, notInterruptible, isTradeSkill, isEmpowered)
    self.__barType = {
        standard = true,
        channel = isChannel,
...
function HUI_CastingBarMixin.OnUpdateBarFillTexture(self, isFull)
    HUI_CastingBarMixin.SetCustomColor(self)
end

function HUI_CastingBarMixin.SetHooks(frame)
    if frame.SetLook then
        hooksecurefunc(frame, "SetLook", HUI_CastingBarMixin.SetLook)
        HUI_CastingBarMixin.SetLook(frame)
    end
    if frame.UpdateIconShown then
        hooksecurefunc(frame, "UpdateIconShown", HUI_CastingBarMixin.UpdateIconShown)
    end
    if frame.SetStatusBarTexture then
        hooksecurefunc(frame, "SetStatusBarTexture", HUI_CastingBarMixin.SetStatusBarTexture)
    end
    if frame.HandleCastStop then
        hooksecurefunc(frame, "HandleCastStop", HUI_CastingBarMixin.HandleCastStop)
    end
    if frame.GetTypeInfo then
        hooksecurefunc(frame, "GetTypeInfo", HUI_CastingBarMixin.GetTypeInfo)
    end
    if frame.AddStages then
        hooksecurefunc(frame, "AddStages", HUI_CastingBarMixin.AddStages)
    end
    if frame.UpdateStage then
        hooksecurefunc(frame, "UpdateStage", HUI_CastingBarMixin.UpdateStage)
    end
    if frame.ShowSpark then
        hooksecurefunc(frame, "ShowSpark", HUI_CastingBarMixin.ShowSpark)
    end
    if frame.PlayFinishAnim then
        hooksecurefunc(frame, "PlayFinishAnim", HUI_CastingBarMixin.PlayFinishAnim)
    end
    if frame.SetColorFill then
        hooksecurefunc(frame, "SetColorFill", HUI_CastingBarMixin.SetColorFill)
    end
    if frame.OnUpdate then
        frame:HookScript("OnUpdate", HUI_CastingBarMixin.OnUpdate)
    end
    if frame.AdjustPosition then
        hooksecurefunc(frame, "AdjustPosition", HUI_CastingBarMixin.AdjustPosition)
    end
    if frame.UpdateHighlightImportantCast then
        hooksecurefunc(frame, "UpdateHighlightImportantCast", HUI_CastingBarMixin.UpdateHighlightImportantCast)
    end
    if frame.GetEffectiveType then
        hooksecurefunc(frame, "GetEffectiveType" , HUI_CastingBarMixin.OnGetEffectiveType)
    end
    if frame.PlayInterruptAnims then
        hooksecurefunc(frame, "PlayInterruptAnims", HUI_CastingBarMixin.OnPlayInterruptAnims)
    end
    if frame.UpdateBarFillTexture then
        hooksecurefunc(frame, "UpdateBarFillTexture", HUI_CastingBarMixin.OnUpdateBarFillTexture)
    end
end