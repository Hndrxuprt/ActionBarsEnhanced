...
ABE_CDMCustomAuraFrameMixin = CreateFromMixins(ABE_CDMCustomAuraMixin)

function ABE_CDMCustomAuraFrameMixin:ConfigureAuraButton(auraButton)
	...
	if not auraButton.iconTexture then
		...
		ABE_CDMCustomFrameCustomized:SetupBackdrop(auraButton.iconTexture.iconBorder, frameName, color)
...
function ABE_CDMCustomAuraFrameMixin:CustomizeAuraButton(auraButton)
	local frameName = self.frameName

	if auraButton:CanBeAccessedInContext() then
		ABE_CDMCustomFrameCustomized:CustomizeCooldownFrame(auraButton.cooldownFrame, frameName)
		ABE_CDMCustomFrameCustomized:RefreshAuraColor(auraButton.cooldownFrame, frameName)
		ABE_CDMCustomFrameCustomized:RefreshAuraTimer(auraButton.cooldownFrame, frameName)
		ABE_CDMCustomFrameCustomized:CustomizeCooldownFont(auraButton.cooldownFrame, frameName)
		ABE_CDMCustomFrameCustomized:CustomizeStacksFont(auraButton.countText, frameName, auraButton)
		if Addon:GetValue("UseCDMBackdrop", nil, frameName) then
			if not auraButton.iconTexture.iconBorder:HasAnyForbiddenAspects() then
...
function ABE_CDMCustomAuraFrameMixin:GetPlaceholder(index)
	...
	if not placeholder then
		...
		ABE_CDMCustomFrameCustomized:SetupBackdrop(placeholder.icon.iconBorder, self:GetName())
	end
	return placeholder
end

function ABE_CDMCustomAuraFrameMixin:UpdatePlaceholder(placeholder, spellID)
	placeholder:SetSize(self.iconSize, self.iconSize)
